...
The original gate used `readStoredField(atom, 'thing')`, which returns null for duplicate valid `thing` fields. Boundary classification instead inspects the first `thing` field. RED: with a sealed/proven `Backup` and an unsealed COW sibling `{ 'thing@backup@default':'Other', thing:'Alias', situation:'', slot:[], strut:[] }`, `collectDefaultBackupBoundary` raised `AMBIGUOUS_DEFAULT_BACKUP` while the declaration gate returned false. The exact focused `--test-name-pattern='a second typed backup with duplicate thing fields invalidates archive reuse'` command failed 0/1, exit 1 (`false !== true`).

The gate now uses the first valid private field descriptor rather than requiring exactly one. This catches every typed root recognized from a valid first `thing` field by boundary, while an earlier malformed field can only cause an extra conservative full-walk fallback. No boundary proof, authorization, revision or public API changed. The focused test is GREEN 1/1, exit 0. Final direct focused/boundary/field/Agent suite: 30/30, exit 0; selected restore/parallel rename 4/4, Agent move/delegation denial 2/2, public Program/backup 6/6, old-reader memory 1/1, all exit 0. New fixtures retained; backups disabled. This test proves the fallback gate matches the reviewer shape; it does not assert an observed permission bypass in the pre-fix code.
