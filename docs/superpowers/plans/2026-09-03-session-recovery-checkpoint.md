...
- **当前实施**：旧生成print迁移候选500e166已两轮任务复核通过。完整私密副本040497d维护提交/重复/回滚/再迁移全通过；未变planner与运行时普通CLI warm/cold各2个真实打印通过，手写body负例拒绝。R2新增证据窗口绑定及定向回归，18pass/1 Windows symlink skip；正进入最终分支复核/门禁/生产维护，生产未迁移。共同设置真实基准与真机验收仍局部待定。
