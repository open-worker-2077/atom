...
- **生产RED**：用户在正式 `manage` 入口右键长按后仍看到旧 F 式全场沉浸，没有完整显式的 `manage` 外围域壳；原测试只验证进入路径和内部节点，未验证域壳边界位于视口内。
- **修复边界**：复用现有域壳、名称、内部节点和 `clusterDomainFrame`，只修正长按沉浸完成后的取景，不改右键仲裁、普通 A 剖开、Graph 数据或 F 历史入口。
- **验收合同**：长按进入的当前域壳四边均保留可见安全边距，团外节点隐藏，内部节点仍显示；从正式入口回读后才关闭。
- **候选GREEN**：正式旧版以2514×1316只读回放复现域壳半径2424px且中心落在视口外，稳定后又缩为181px并偏右，证实旧取景只框内部节点。候选改为长按进入及权威子域首次载入后均复用现有`clusterDomainFrame`框住当前域壳；直接关联168/168、Chromium长按关键旅程4/4通过，其中超宽画面要求域壳完整可见、半径至少占短边30%且中心误差不超过视口8%。GitNexus识别8个变更符号、0个受影响流程，风险LOW；尚待集成部署及正式入口回读。
- **正式回读**：候选`180bce6`快进主线并经既有Runtime/Watchdog入口部署，4784新PID41160、revision7479、projection published，正式`spatial-engine.js`与main SHA-256均为`23b077a0…221c8`。2514×1316只读长按`atom.json → 🧊manage`后，`manage`域壳中心精确为(1257,658)、半径539.56px，内部仅显示`包办`、`办包`，共同边框配置100%；生产`atom.json` SHA-256仍为`3a9e51cb…afcbb`。独立远端备份分支`origin/fix/a-immersive-domain-shell@180bce6`已建立；远端main更新由自动审批独立拦截，不影响本机E3交付。
- **长按续发缺陷候选（2026-09-07）**：用户正式画面继续暴露两项相连问题：PageDown展开后沿用旧点击点导致当前外围团偏移，且每次只查锚定团直属节点而不能连续下钻。真实RED分别得到纵向偏移501.7px及第二次PageDown未出现下一层。候选保持首次十字锚定范围，直到真实鼠标移动或重新点击；每次PageDown沿该范围内当前可见末端子团各展开一层，并以现有中键框团取景重新居中。Node直接链90/90、Chromium相邻A旅程10/10、build/control通过；待部署及正式入口只读回读。
- **长按续发正式回读（2026-09-07）**：候选`ff46275`快进主线并经既有Runtime/Watchdog入口部署，正式新listener PID39536、health revision7480、projection published，服务代码与main SHA-256均为`03c0e20d…2eb6`。2514×1316只读回放`atom.json → 🧊manage → PageDown×2`，可见域团由1增至3再增至7，证明同一锚定团可连续下钻；三阶段`manage`外围团中心均为(1257,658)、半径539.56px，未再沿旧点击点偏移。生产`atom.json`哈希保持`3a9e51cb…afcbb`。首次受控重启在30秒等待窗口内未健康，watchdog随后于11:38:29恢复正式listener；这是部署等待时序，不是产品运行失败。

**当前续点（2026-09-06）**：Task1—7的代码、部署和手势证据保留，但最终外围团画面未通过用户真实`manage`验收。2026-09-06截图显示长按后仍退化为旧F式全场沉浸，没有显式当前域壳；此前“完成、不重做A”的结论错误。当前按生产缺陷重开：修复后必须从同一正式入口验证当前域壳、名称、内部节点同时可见且团外隐藏，不能再以资产一致、健康检查或手势触发替代画面验收。
