...
- **Stage 4 formal deployment**: attempt `prod-20260907-identity-26a0219-r2` committed revision `472f1097…e981` with a verified private backup and receipt-bound rollback. Postflight is idempotent (`changed:false`), all `12,243` Things retain unique identities, topology is preserved, and all 81 active resolvable Strut endpoints are bound. The restarted 4784 reports revision 7485 and published projection at the same world revision. Public Web state contains semantic `manage` data with no `&id`; the formal Agent CLI reads its own semantic path successfully, while an out-of-window read remains denied.
- **Next**: the permanent-identity mainline is complete. Execute the queued Web pointer/crosshair same-focus display revision.
