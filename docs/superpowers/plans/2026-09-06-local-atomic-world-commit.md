...
Fix round 1 is VERIFIED from evidence revision `a256dfa9b543ac2d97a4d7d35b02ccd981856ce1` (fix SHA recorded after commit). A deterministic second-owner barrier after rollback candidate preparation produced RED `1 !== 2`: inverse facts rebased after the owner committed a disjoint path, but `rebaseResult` was not retained and the compatibility manifest was not recalculated. Passing the existing callback into candidate preparation makes the focused manifest/facts journey `1/1` GREEN (`85.247ms`). The public journey now keys histories by correlation ID and compares each complete `changedPaths` and patch-operation path array to its sole expected path, with explicit peer-path absence and distinct command IDs; focused public GREEN is `1/1` (`11,021.552ms`). The exact affected gate passes `198/198`, zero failures/skips, in `35,633.623ms`; original private-copy performance and production-source conservation evidence remain unchanged.
