...
- [x] **Step 1: Write failing contract tests**：把一个Program自身作为Strut后项，断言`{}`会接收一次delivery；加入另一个未作为后项的Program，断言不运行；断言旧`{"nodes":[...]}`返回合同校验失败。
- [x] **Step 2: Run RED**：运行`node --test --test-name-pattern="receiver-owned|rejects strut nodes" tests/atom-program-runtime-scheduling.test.mjs`，确认失败来自当前worker仍要求`nodes`和索引仍绑定参数节点。
- [x] **Step 3: Implement minimal worker contract**：在`extract_trigger_contract()`中仅允许Strut空对象参数，保留单delivery形参校验，并返回空`parameters`。
- [x] **Step 4: Implement receiver-owned index**：`setTriggerContract/removeTriggerContract/backfillTriggerIndexForEvent`对Strut统一使用Program自身path；普通delivery只查询`strut\0${consequentPath}`，不读取参数节点。
- [x] **Step 5: Run GREEN**：重复Step 2并运行完整`tests/atom-program-runtime-scheduling.test.mjs`。
- [x] **Step 6: Commit**：提交`feat(program): make strut triggers receiver-owned`。
...
- [x] **Step 1: Write failing slot tests**：把测试槽模的Strut后项改为`计算`或`接棒`Program角色、Trigger改为空参数；证明当前实例运行、兄弟实例不运行、普通事实后项不唤醒任意共享Program。
- [x] **Step 2: Run RED**：运行两个槽体测试文件的receiver-owned用例，确认失败来自运行时仍用`contract.parameters.nodes`筛选来源角色。
- [x] **Step 3: Implement minimal role mapping**：Strut事件先解析实例`consequentPath`为采用修订角色；仅当该角色`kind === "program"`时映射其role path到槽模Program，忽略其他Program角色；Transform事件继续保留既有`nodes`合同。
- [x] **Step 4: Run GREEN**：运行槽体两个测试文件，确认锁、claim、回滚、重试和re-seal行为保持。
- [x] **Step 5: Commit**：提交`feat(slot): dispatch strut to consequent program roles`。
...
- [x] **Step 1: Write failing planner tests**：覆盖唯一旧node到唯一Program、同一node多个Program扩为多个显式后项、相对槽角色、归档Program跳过、无对应clause与动态参数稳定阻断、源facts不被修改。
- [x] **Step 2: Run RED**：运行`node --test tests/atom-strut-receiver-migration.test.mjs`，确认模块缺失。
- [x] **Step 3: Implement AST source rewrite**：Python只接受顶层literal Strut Trigger，输出旧nodes、entrypoint和`{}`改写源码；不以正则改Python。
- [x] **Step 4: Implement pure migration plan**：索引活跃Program与Strut后项，把每个旧node对应的delivery后项替换/扩展为订阅Program exact path；保持ordinal、if、普通未订阅后项和归档子树；重新投影并计算expected/next revision。
- [x] **Step 5: Implement guarded deploy script**：复用现有inline-strut部署模式，执行预检、revision复验、世界目录备份、中央原子替换、投影恢复与回读；任何阻断不写事实。
- [x] **Step 6: Run GREEN**：运行迁移测试及现有migration/Graph投影相关测试。
- [x] **Step 7: Commit**：提交`feat(migration): convert strut subscriptions to graph endpoints`。
...
- [x] **Step 1: Migrate fixtures by behavior**：每个测试Graph把实际接收Program设为then后项；不做机械字符串替换，不把无关普通事实继续当隐式接收端。
- [x] **Step 2: Update Help**：公开`trigger("strut", {}, main)`，明确Graph后项必须是接收Program；delivery结构保持不变。
- [x] **Step 3: Verify affected chain**：运行Program scheduling、inline Strut、Slot body、Strut lock、Slot signal与迁移测试。
- [x] **Step 4: Verify system**：运行`npm run test:system`；随后运行`npm test`、Python AST、Node syntax和`git diff --check`。
