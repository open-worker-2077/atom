...
- **当前功能项**：通用点击次数与 Program 声明三点击运行的设计已获用户批准并写入 Web 空间规格；下一步编写实施计划并按 TDD 实现，输入层不得硬编码三点击用途。
...
- [ ] **Task 4 — 通用点击次数实现**：brainstorming 已批准且规格已固化；编写实施计划后按 TDD 实现，三点击运行由 Program 声明。
