...
- **叶级声明**：只有最细级槽位显式携带专用Thing值标记；从该叶槽位回溯至候选DataFlow根的中间父节点自动成为结构槽位。叶槽位不得再包含下级槽位。
...
- **专用标记候选**：建议在Thing值开头使用版本化保留前缀`⟦atom.slot:v1⟧`，例如`{"thing":"⟦atom.slot:v1⟧联系电话"}`。`⟦atom.`命名空间由内核保留，普通应用名称不得以其开头；封装器只精确识别完整前缀，显示层可隐藏前缀而保留净名称。该字面量在用户确认前只作为推荐，不进入实现。
- **槽料与预置**：最细级槽位下未携带槽位标记的普通Thing子树才是槽料；打印内核不复制模板槽料。复杂或简单预置均由应用Program在槽例建立后生成，不增加内核默认值分支。
