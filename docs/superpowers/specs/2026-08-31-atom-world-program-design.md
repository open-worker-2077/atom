...
- **槽料与预置**：末级槽位下未携带槽位标记的普通Thing子树才是槽料。预置槽料不等于槽模静态默认值：应用Program可在槽例建立后读取槽模外Atom的Situation，解析、判断和筛选后，在对应槽位下生成允许读取、禁止改写的槽料；内核不解释预置业务内容。
- **推支唯一触发候选（仅）**：不建设脱离Graph的事件发布／订阅总线，现实发生内容以`atom.json`已提交事实为准。Transform变化只供运行时定位相关作用关系，不直接触发Program；Slot标签只沿实际推支关系流动，不能脱离推支线全局广播。普通或复合推支线本身是内含Program的加工单元：线内Program接收上游标签，可结合revision绑定的Atom事实统筹判断，并自行决定是否调用`slot()`以及向其已声明的下游端点传递什么标签；不调用即在该线终止。下游节点按实际到达的标签订阅选择本节点Program作为运行起点，因果来源仍是该条推支线。标签只作控制与选择元数据，不替代`atom.json`业务事实。该方向本轮仅研讨和登记，不进入开发；多输入归并、一次运行可发送的标签组数、循环抑制与现有Transform／Slot触发兼容迁移仍待定界。
