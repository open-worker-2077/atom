...
- **末级声明**：只有最细级槽位显式携带专用Thing值标记；从该末级槽位回溯至候选DataFlow根的中间父节点自动成为结构槽位。槽位是固化结构中的枝丫，槽料才是其下的叶；末级槽位不得再包含下级槽位，但可以包含多层槽料子树。
- **写入边界**：Agent只在最细级槽位内创建／修改槽料；自动推导出的父级槽位只组织下级槽位，不允许直接混入与下级槽位同层的普通槽料。结构变化继续通过槽模修改与reseal完成。
- **Situation语义**：槽位Situation只解释槽位本身及其用途、约束，不充当槽例业务值或默认槽料。
- **专用标记定论**：Thing值开头使用保留前缀`⟦atom.slot⟧`，例如`{"thing":"⟦atom.slot⟧联系电话"}`。`⟦atom.`命名空间由内核保留，普通应用名称不得以其开头；槽体版本由不同槽体及槽模修订控制，不在槽位标记内增加版本号。标记作为槽位边界永久显式保留，CLI、Agent与执行器据此识别并拒绝把槽位当槽料改写；展示层可以另行提供净名称，但不得删除权威Thing中的标记。
...
- **创建触发候选**：建议由槽例print成功提交产生持久化“槽例已创建”事件，显式订阅该事件的槽模Program以新槽例为scope运行。print与各Program结果分别构成局部原子提交；初始化失败不回滚槽例，保留可诊断、可重试的待初始化状态。事件必须随print提交耐久登记，并按槽例稳定身份、采用的槽模修订、Program角色和创建事件去重，避免崩溃恢复后漏跑或重复落料。具体触发字面量、并行/依赖顺序及只读锁定时点仍待研讨定论。
