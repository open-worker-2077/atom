...
- **负责**：定义世界事实、contain/support/Shortcut 关系、Transform、Program 执行合同、槽体闭环和备份域语义。
...
- **Contain**：定义真实层级和权限传播路径；路径变化通过中央事务更新，不能由显示缓存推断。
- **Support**：保存可审计的前项、后项、判定 Program 与 clause 身份；普通 Thing 只提供事实，不保存布尔职责。
...
- **职责分离**：普通前项和后项只提供事实；support 引用的独立 `thing@program` 是唯一 strict-bool 判定者。
- **严格结果**：判定结果必须是 `true` 或 `false`；缺少判定 Program、非布尔返回或执行失败均不得默认成 `true`。
- **显式交付**：`true` 只形成绑定候选 revision 的 typed delivery；只有后项自身显式声明 `trigger("support", ...)` 的 Program 才接收。
...
- **同体权限**：seal、print、局部触发和 reseal 必须在实际槽体 contain 边界内由获权 Program 完成。
...
- **权限不转借**：Shortcut 不授予目标 ACL，不绕过 Agent 的实际 contain 路径裁定。
...
- **关系完整**：子树、support、Shortcut 引用与中央可逆记录共同提交，恢复后能按当前合同重新投影。
...
- **稳定拒绝**：非法 support、原生 N→M、非 strict bool、目标冲突、越权和失效 Shortcut 返回可区分错误码与上下文。
...
- **Support 矩阵**：true、false、非布尔、异常、无订阅、精确订阅、订阅失败均验证职责、交付和回滚。
