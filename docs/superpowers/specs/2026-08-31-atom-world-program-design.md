...
- **推支唯一触发候选（仅）**：不建设脱离Graph的事件发布／订阅总线，现实发生内容以`atom.json`已提交事实为准。Transform变化只供运行时定位相关作用关系，不直接触发Program；Slot标签只沿实际推支关系流动，不能脱离推支线全局广播。普通或复合推支线本身是内含Program的加工单元：线内Program接收上游标签，可结合revision绑定的Atom事实统筹判断，并自行决定是否调用`slot()`以及向其已声明的下游端点传递什么标签；零次输出隐含false，一次或多次标签输出隐含true及其内部细分，不再另传布尔值。最简应用范式是恒等转发：线内Program不作业务判断，把当前因果批次内全部上游标签原样打包传给下游；它只是应用Program可选写法，不是内核强制模式。复合推支只汇总同一因果批次的输入，不暗存历史标签；需要跨批次等待或累计时必须写入`atom.json`。下游节点按实际到达的标签订阅选择本节点Program作为运行起点。标签只作控制与选择元数据，不替代`atom.json`业务事实。该方向本轮仅研讨和登记，不进入开发；一次运行可发送的标签组数、循环抑制与现有Transform／Slot触发兼容迁移仍待定界。
