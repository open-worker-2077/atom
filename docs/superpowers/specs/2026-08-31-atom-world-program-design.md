...
- **推支唯一触发候选（仅）**：不建设脱离Graph的事件发布／订阅总线，现实发生内容以`atom.json`已提交事实为准。Transform变化只供运行时定位需要重新评估的出向推支线，不直接触发Program；Slot标签也只作同体接口中的选择／路由元数据，不能单独构成因果触发。普通或复合推支线的应用Program可结合前序标签与revision绑定的Atom事实作统筹判断，但向后项只交付严格布尔值；`true`才允许后项节点按本地标签订阅选择Program作为运行起点，`false`不运行。标签不得承载业务事实或绕过推支线全局广播，后续复杂控制及是否继续向出向推支线提供标签由应用Program负责。该方向本轮仅研讨和登记，不进入开发；复合输入归并、循环抑制与现有Transform／Slot触发兼容迁移仍待定界。
