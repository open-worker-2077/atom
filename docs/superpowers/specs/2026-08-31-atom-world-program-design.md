...
- **通用事件候选（仅）**：事件管理应是独立、小型、通用的内核局部子框架，而非由槽体内核写死“槽例已创建”等少数事件。应用Program负责事件名称、载荷、业务身份、去重、是否触发、重试、槽料来源凭据和只读约束；内核候选只提供事件发布／创建、监测／订阅、权限与作用域裁定及提交边界，`trigger`只是消费事件的入口之一。该方向本轮仅研讨和登记，不进入开发；持久或临时投递、确认语义及其他消费入口以后单独定界。
