...
test('3d graph orientation settings wrap full rotations and bound branch spread', () => {
  const model = loadModel();
  ...
  assert.equal(model.withEmptyNodeDiameterInput(defaults, 80).emptyNodeDiameterPercent, 80);
  assert.equal(model.withEmptyNodeDiameterInput(defaults, 0).emptyNodeDiameterPercent, 20);
  assert.equal(model.withEmptyNodeDiameterInput(defaults, 500).emptyNodeDiameterPercent, 200);
});
