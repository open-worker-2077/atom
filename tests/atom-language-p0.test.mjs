...
test('9a. migration parsing keeps the legacy kernel identity explicit', () => {
...
test('9b. external Atom requests cannot supply a kernel Thing identity', () => {
  ...
  const nestedEndpoint = createAtomLanguageReceiver().receive(
    'transform {"thing":"来源","strut":[{"if@current":true,"then":[{"thing&id=00A":"目标"}]}]}'
  );
  assert.equal(nestedEndpoint.ok, false);
  assert.equal(nestedEndpoint.errors[0]?.code, 'KERNEL_IDENTITY_INPUT_FORBIDDEN');
});

test('9c. short Thing identity survives field replacement', () => {
  const atom = { 'thing@program&id=00A#窗口': '石器工坊' };
  const first = storedField(atom, 'thing').parsed.identity;
  assert.equal(first, '00A');
