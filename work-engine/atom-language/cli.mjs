...
function help() {
  ...
  return [
    ...
    '  日常 Agent 管理：目标必须位于一个真实 contain 上级 Agent 的后代写域；上级用公开 CLI 对目标 Program 做完整 situation.rep，新的 labels／groups／names 仍按上级已持有 scope 进行委派校验。管理 Agent 不得修改自身、普通父节点或平行域，也不能借重配提高自身权限。',
    '  一次性 bootstrap：只有首次启动、灾难恢复或必要迁移可由受信 maintenance 创建最小管理 Program、把孤立目标重挂到其 contain 下、运行 agent() 并 exact／冷启动验证；失败按中央 receipt 补偿，成功后 maintenance 必须退出日常管理链。管理 Program 与授权仍只以 Program 源码和 Graph 事实为权威。',
