...
function help() {
  ...
  return [
    ...
    '  世界函数：explore(query)->rows；transform(spec)、agent(spec)、slot_body(spec)、lock(spec)、message(spec)->effect；current_atom()->Program。',
    '  Agent登记：当前 Program 调用 agent({"labels":["^^","业务标签"],"functions":{"groups":["form"],"names":["message"]}}) 即把本节点登记为 Agent；无 target／lock／mode。functions 必填且禁止 null、通配；groups 在登记时按当前注册表展开，存量窗口只保存固定去重 names。',
    '  标签边界：连续 ^ 只表示管辖等级，普通字符串是业务标签，两者不混算；自身或子 Agent 的 ^ 数量不得超过创建者。持有 ^ 的 Agent 可在既有 Graph 活动空间内定义普通标签；子 functions 必须是创建者实际固定 names 的子集。',
    ...
    '  一次性迁移：新运行时仅认 agentRegistrations 固定安全上下文；旧 request-driven 侧车中的 windowSelfLocks 或 windowSelfLockAgents 稳定返回 RETIRED_WINDOW_SELF_LOCK_SNAPSHOT，必须由受信 bootstrap 一次性映射，不保留双权限 ABI。',
    ...
    '  业务状态锁：lock() 只触发内核已登记的相对范围／Explore或Transform动作／标签模式，不能覆盖固定 Agent／槽体锁。条件优先复用既有 support 推线的布尔结果，避免重复大范围 Explore。',
    ...
    '  “任务区”必须是当前窗口下已获准写入的普通事实父节点；不要通过给窗口自身追加 contain 绕过固定锁。两步均须使用当前已认证 --agent 并走统一 Graph 权限域；第2步成功回执后再 exact explore 回读新 Agent 与推进流。functions 可改用登记时冻结的 groups，但应优先选择最小 names；不得用公开 Transform 创建 thing@agent。',
    '',
    '反馈：',
