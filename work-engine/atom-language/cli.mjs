...
const TRANSFORM_HELP = Object.freeze({
  ...
  mov: '{"name.mov.DESTINATION_PATH":"A"}（移动子树，拒绝形成循环）',
...
function help() {
  ...
  return [
    ...
    '  “世界之外”以 name@universe 暴露为不落盘的虚拟父级；它只用于读取、上下钻和顶层消歧。',
    ...contract.explore,
    ...
    '  AMBIGUOUS_ATOM_NAME / ATOM_NOT_FOUND：重新 explore；使用能唯一表征目标的最短 exact 路径。',
