...
function help() {
  ...
  return [
    ...
    '  槽体封装：运行槽体自身 Program 即自声明封装；调用方不得传 body，内核也不按名称识别或改名候选 DataFlow。中央事务原名保留候选，并新增 print@program 与槽例；不预建空槽例，print 计划可 exact explore 审计。',
    ...
    '  槽体打印：授权调用方使用 use_program({"name":"EXACT槽体/print","arguments":{"name":"新槽例名"}})；name 是唯一打印参数，修订由 print@program 绑定。内核复制抽象槽、嵌套 slot、strut、类型、槽契约与默认初值，不复制应用 Program 或后续具体料。',
    '  槽例填写与计算：槽例数据事件按“当前槽例→稳定角色→槽模共享逻辑”只执行实际触达链。槽模内相对引用和内部绝对引用均映射为当前槽例数据，结果写回当前槽例；槽模外绝对引用保持真实外部目标并继续经过统一鉴权。禁止跨槽例和穿透嵌套槽例边界。',
