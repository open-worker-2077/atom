...
function help() {
  ...
  return [
    ...
    '  虚拟引用：target = explore({"thing":"EXACT目标"})[0]；shortcut({"placement":"contain","thing":"显示名","target":target})在当前 Program 直接 contain 下创建引用。reference 必须是已授权父 contain Explore 返回的精确 shortcut ThingCoordinate；shortcut({"action":"delete","reference":reference})只删除引用，不改变目标，也不删除创建 Program。两种操作都不接受路径字符串或 .ref。引用不复制目标事实、不携带创建者权限；每次查看均以本次 Agent 的普通 Explore 鉴权解析目标。首版 Transform 不经引用重定向。',
