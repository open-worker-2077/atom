...
function help() {
  ...
  return [
    ...
    '  每条 rule 必须且只能含一个 @current:true；if 永远是前项，then 永远是后项。禁止无 current 与线载源码；同一 rule 可声明 N→M，事实前项与独立判定 Program 保持分层。',
    '  Program 端点写 {"thing@program":"selector"}：前件 Program 仅以 strict bool 决定本条支撑，且不得产生写入等副作用；后件 Program 只按自身触发/use_program/显式运行计算自己。',
    '  N→1、1→N 与 N→M 直接保留同一 support clause 身份；Web 在归一化 0.5 形成共享汇流／分流线干。仅当业务语义本身需要可审计枢纽 H 时，才显式拆为 N→H 与 H→M。Program 源码只放 exact thing@program 节点的 situation。',
