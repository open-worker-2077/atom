...
function help() {
  ...
  return [
    ...
    '  推支触发器：普通前项／后项不保存布尔值；推支线 if 内嵌 Program strict true 后只形成 typed delivery。Graph then 必须把实际接收 Program 写为 thing@program 后项；该 Program 以 trigger("strut", {}, main) 声明是否响应，其中 main(delivery) 接收 decision、clauseId、antecedentPaths、consequentPath 与 revision。Program 不得用 nodes 重复选择 Graph 后项；false、普通事实后项或未声明 Trigger 均不执行。',
    ...
    '  槽体研发：槽体本身是 thing@program，Situation 内调用 slot_body({"action":"seal"})，首次只含一棵普通可自运行候选 DataFlow（下级槽、slot、strut、@program）。研发态可用 transform {"thing.run.EXACT候选根路径":"EXACT_PROGRAM_PATH"} 绑定当前域。',
    '  槽体封装：运行槽体自身 Program 即自声明封装；调用方不得传 body，内核也不按名称识别或改名候选 DataFlow。中央事务原名保留候选，并新增 print@program 与槽例；不预建空槽例。print 计划可用 explore {"thing":"EXACT槽体/print/修订","slot$latitude-1":true} 审计。',
    ...
    '  槽体打印：授权调用方使用 use_program({"name":"EXACT槽体/print","arguments":{"name":"新槽例名"}})；name 是唯一打印参数，调用方不得传 revision，修订由当前 print@program 内部绑定。内核复制抽象槽、嵌套 slot、strut、类型、槽契约与默认初值，不复制应用 Program 或后续具体料。',
    '  槽例填写与计算：使用 transform {"thing":"EXACT槽体/槽例/实例/槽","situation.rep.填写值"} 写入槽例数据。槽例数据事件按“当前槽例→稳定角色→槽模共享逻辑”只执行实际触达链；研发态绑定支持 ./相对 slot 路径。槽模内相对引用和内部绝对引用均映射为当前槽例数据，结果写回当前槽例；槽模外绝对引用保持真实外部目标并继续经过统一鉴权。禁止跨槽例和穿透嵌套槽例边界。',
    '  槽例填料与变量：带槽模角色的实例节点是抽象槽；槽下 transform new 的未映射普通 Thing 子树是本地料。Program 与文本节点分工：Program 保存共享逻辑，正式业务数据由槽例文本／事实节点提供；不同槽例调用上下文隔离。',
    '  槽模修订：同一槽体 Program 在同一中央事务内修改自身候选 DataFlow 后再次 slot_body({"action":"seal"})；只有该自声明组合获得 slot-reseal 能力。成功后同步全部槽例的映射结构与共享 Program 引用，并逐字节保留未映射本地料；删除含料槽整次冲突回滚。',
