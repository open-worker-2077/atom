...
- 旧 Program 通过一次性、结构化源码升级改用新四轴调用：只修改可由 Python AST 与词法块定义使用链唯一证明的 Graph API 字典键和 AtomView 访问；语法上使用变量不等于语义动态，能唯一归约且在同一执行块中支配单一结构消费的常量可生成最小 edit，普通字符串、业务计算与真正动态表达式不改；无法唯一升级者进入精确阻断清单。
