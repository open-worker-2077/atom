...
#### Scenario: 守窗默认态跨请求保持
- **WHEN** 无显式 `lock` 的有效 `jump` 因 `when=false` 守在原位，随后以同一窗口发起新的独立 CLI 请求或重建调度器
- **THEN** request-driven snapshot 保持该窗口的自锁激活路径，默认策略仍由当前位置计算；系统不得重放缓存 Program 的历史 Explore 依赖并把它误作本次请求读取
