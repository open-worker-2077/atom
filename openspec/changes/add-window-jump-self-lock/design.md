...
Activation paths live in the existing request-driven lock snapshot as `windowSelfLockAgents`; only default-active paths without an explicit policy need this compact entry. Explicit overrides remain separate `windowSelfLocks` records keyed by locked Agent path, and their paths also imply activation so snapshots do not duplicate them. Jump atomically remaps the relevant activation/policy key when it moves that same Agent. A same-window replacement is accepted only if set inclusion proves that every newly allowed coordinate was already allowed; priority/rule edits and relation-depth changes are reduced to the same effective-set comparison on the candidate tree.

The scheduler's `current()` and cached/reusable projection paths overlay both active window paths and their explicit policies onto every public request cycle. The engine therefore reconstructs the same access controller after a request boundary without rerunning the jump Program. Historical `exploreReadPaths` in a cached projection describe already-completed Program work and are not re-authorized as reads of the later public request; only Programs actually executed in the current cycle receive Program-read preflight, while the public exact target always receives the ordinary access check. An exact target denied specifically by the window self-lock is returned as `WINDOW_ACCESS_DENIED`; ordinary Program/node protection retains its non-disclosing protected-result behavior.
