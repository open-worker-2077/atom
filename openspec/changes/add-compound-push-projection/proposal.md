...
- N→1 在归一化路径 50% 汇合后共享一条干线，1→N 在 50% 分叉前共享一条干线。
- 禁止一个 support clause 原生表达 N→M；需要多入多出时必须建立可见、可审计的真实枢纽 Thing `H`，拆为 N→H 与 H→M 两条独立 clause。Web 可让两段几何连续，但不得隐藏 H 或合并 clause identity。
